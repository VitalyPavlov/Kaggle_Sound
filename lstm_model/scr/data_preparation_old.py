...
def save_npy(df, function, name='train', label=0, place=None):
    for ind in tqdm(range(df.shape[0])):
        ...
        num_labels = df.loc[ind, 'num_labels']
...
def main():
    ...
    mask = train_curated.num_labels != 1
    first_label = train_curated['labels'].apply(lambda x: label_mapping[x.split(',')[0]]).values
    last_label = train_curated[mask]['labels'].apply(lambda x: label_mapping[x.split(',')[-1]]).values

    for function in functions:
        label = 0
        label = save_npy(train_curated, function, name='train', label=label, place='first')
        _ = save_npy(train_curated[mask].reset_index(), function, name='train', label=label, place='last')

    y_train = np.concatenate((first_label, last_label), axis=0)
    ...
    first_label = train_noisy['labels'].apply(lambda x: label_mapping[x.split(',')[0]]).values
